...
public:
    FlywheelComponent() : BotComponent("Flywheel component",
                                       {
                                               ComponentAction::FLYWHEEL_FULL_SPEED, ComponentAction::FLYWHEEL_STOP, ComponentAction::FLYWHEEL_PASSIVE_SPEED
                                       }) {}
    ...
    void Execute(ComponentAction action, int value) override {
        ...
        } else if(action == ComponentAction::FLYWHEEL_PASSIVE_SPEED) {
...
void Components::Execute(ComponentAction action) {
    Execute(action, 0);
}
